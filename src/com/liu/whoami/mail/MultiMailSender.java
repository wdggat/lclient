    ...
    /** 
      ...
      */ 
        public boolean sendTextMail(MultiMailSenderInfo mailInfo) { 
          ...
          String[] toAddrs = mailInfo.getToAddress();
          if (receivers != null){
              // 为每个邮件接收者创建一个地址
              tos = new InternetAddress[receivers.length + toAddrs.length];
              for (int i=0; i < toAddrs.length; i++)
                  tos[i] = new InternetAddress(toAddrs[i]);
              for (int i=0; i<receivers.length; i++){
                  tos[i+1] = new InternetAddress(receivers[i]);
              }
          } else {
              tos = new InternetAddress[toAddrs.length];
              for (int i=0; i < toAddrs.length; i++)
                  tos[i] = new InternetAddress(toAddrs[i]);
          }
...
//            Address[] tos = new InternetAddress(mailInfo.getToAddress());
            Address[] tos = new InternetAddress[toAddrs.length];
            for (int i=0; i < toAddrs.length; i++)
                tos[i] = new InternetAddress(toAddrs[i]);
            mailMessage.setRecipients(Message.RecipientType.TO, tos);
